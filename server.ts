...
Available Categories & Subcategories: ${JSON.stringify(availableCategoriesList)}`;
  ...
  // Cloudinary Upload API endpoint (handles images, PDFs, documents, media)
  app.post('/api/cloudinary/upload', async (req, res) => {
    try {
      const { file, folder, resourceType, fileName } = req.body;
      if (!file) {
        return res.status(400).json({ error: 'No file data provided' });
      }

      const cloudName = process.env.CLOUDINARY_CLOUD_NAME || process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || 'kpb5rcow';
      const apiKey = process.env.CLOUDINARY_API_KEY || '149468495256154';
      const uploadPreset = process.env.CLOUDINARY_UPLOAD_PRESET || process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET || 'healnex_products';

      const targetResourceType = resourceType || 'auto';
      const targetFolder = folder || 'healnex_uploads';
      ...
      const cloudinaryUrl = `https://api.cloudinary.com/v1_1/${cloudName}/${targetResourceType}/upload`;
      ...
      console.error('[Cloudinary Server Exception]:', err);
      return res.status(500).json({ error: 'Server exception during Cloudinary upload', details: err.message || String(err) });
    }
  });

  // Cloudinary configuration status API endpoint
  app.get('/api/cloudinary/config', (req, res) => {
    const cloudName = process.env.CLOUDINARY_CLOUD_NAME || process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || 'kpb5rcow';
    const apiKey = process.env.CLOUDINARY_API_KEY || '149468495256154';
    const uploadPreset = process.env.CLOUDINARY_UPLOAD_PRESET || process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET || 'healnex_products';
    res.json({
      configured: Boolean(cloudName && apiKey),
      cloudName,
      apiKey,
      uploadPreset,
    });
